...
func TestManager_Enable_ApprovalRecord(t *testing.T) {
	...
	if err := mgr2c.Enable("greeter"); err == nil || (!strings.Contains(err.Error(), "approval record missing or does not match") && !strings.Contains(err.Error(), "requires explicit approval")) {
