...
// RunPluginInstallForTest is an exported wrapper for runPluginInstall (used for WU7 exit verification).
func RunPluginInstallForTest(src, pluginsRoot string, force, yes bool, prompter Prompter, out io.Writer) error {
	return runPluginInstall(src, pluginsRoot, force, yes, prompter, out)
}
