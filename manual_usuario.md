...
- **`agent.max_iterations`** (default de la librería: 10; `forge init`/`forge wizard` escriben 30 en el config que generan): cuántas rondas de llamadas a herramienta puede hacer el modelo **dentro de un solo turno** antes de que el agente lo corte con error (`"turn aborted: agent reached max_iterations..."`). Es *distinto* de `budget.max_iterations` de un manifiesto (RF-11, §10), que es el total acumulado de **toda la corrida**, no de un turno. El default crudo de la librería (10) alcanza apenas para una tarea trivial — se vio en la práctica un manifiesto de una sola tarea agotar las 10 llamadas (fs_list/fs_read/fs_write) antes de poder cerrar el turno; subilo (30-80) si ves ese error. Si tu `.forge/config.json` no fue generado por `forge init`/`forge wizard` (o es de antes de que empezaran a escribir 30), revisalo a mano.
...
forge run --manifest run.json --log         # además, graba toda la actividad en un archivo (ver 10.2)
...
**`--log`**: agrega `--log` a cualquier invocación (`run`, `--resume`, con o sin `--decompose`/`--yes`) para que, además de la terminal, todo lo de arriba (progreso, ticks de herramienta, checkpoints HITL, y el reporte final) se grabe en `--state-dir/.forge/runs/<run_id>/activity.log` — junto a `state.json`/`report.json`, que ese mismo directorio ya guarda. Es *append*, no trunca: correr `--resume --log` sobre el mismo `run_id` sigue agregando al mismo archivo, así el log queda como una sola línea de tiempo continua de la corrida completa. Con `--json` (que apaga los ticks de herramienta y el sondeo de subagentes en la terminal para no ensuciar el JSON de salida), esas líneas *sí* se siguen escribiendo en el archivo — el archivo es el registro completo, la terminal es solo lo que decidís mostrar en vivo.
